  ...
  <body>
        ...
        <div class="row">
            <div class="boxed">
                ...
                <table class="table table-striped">
                    ...
                    ?>
                    <thead>
                        <tr><th>Name</th><th>Creator</th><th>Create Date</th><th></th><th>Download</th></tr>
                    </thead>
                    <tbody>
                        <?php 
    ...
    $('#batchname').keyup(function() {
            ...
            if (!characterReg.test(inputVal)) {
                $('#batchname').before('<p class="text-danger error-keyup"><span class="glyphicon glyphicon-exclamation-sign"></span> No spaces or special characters allowed.</p>');
                $('#startbutton').attr('disabled','disabled');
            }
        });
        
        $('#batchcreator').keyup(function() {
            $('.error-keyup').remove();
            $('#startbutton').removeAttr('disabled');
            var inputVal = $(this).val();
            var characterReg = /^[a-zA-Z0-9]+$/;
            if (!characterReg.test(inputVal)) {
                $('#batchcreator').before('<p class="text-danger error-keyup"><span class="glyphicon glyphicon-exclamation-sign"></span> No spaces or special characters allowed.</p>');
                $('#startbutton').attr('disabled','disabled');
            }
        });
    </script>
